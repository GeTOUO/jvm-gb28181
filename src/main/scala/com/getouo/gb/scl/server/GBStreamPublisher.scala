...
//    val nulHeaderTag =  Array[Byte](0x00,0x28,0x00,0x00)
    val nulHeaderTag =  Array[Byte](0x00,0x00,0x00,0x01)
    websocketActors.writeAndFlush(new BinaryWebSocketFrame(Unpooled.copiedBuffer(data.incStartTag)))
//    websocketActors.writeAndFlush(new BinaryWebSocketFrame(Unpooled.copiedBuffer(data.nalu)))
    packets.foreach(p => {
